    ...
    fileSelected(event) {
      ...
      axios.post('/nova-vendor/array-images/upload', fd)
          .then(res => {
            ...
            app.value = JSON.stringify(app.images.map((item) => item['src']))
          });
    },

    deleteImage(index) {
      if (!this.images || this.images.length === 0 || !this.images[index]) {
        return;
      }
      axios.delete('/nova-vendor/array-images/delete/?src=' + this.images[index]['src'] + '&disk=' + this.field.disk
          + '&path=' + this.field.path).then((res) => {
            if (res.data === 'success') {
              this.images.splice(index, 1);
              this.value = JSON.stringify(this.images.map((item) => item['src']));
            }
      });
    },
